...
- **El backend debe incluir los siguientes endpoints**:
    - **[POST /api/files]**
        ...
        ***Resolusion***
        - 1 Extraer el file del request
        - 2 Validar lo que se envio sea un file
        - 3 validar el mimetype (csv)
        - 4 Transformar el file (buffer) a string
        - 5 transformar el string a json
        - 6 guardar el json en memoria o db
        - 7 devolver el status 200 con el mensaje y el json
    - **[GET /api/users]**
        ...
        - O esta ruta debe devolver el estado 500 y un objeto con la clave "message" con un mensaje de error en el valor.
    - ***Resolusion***
        - 1 Extraer el query param q del request
        - 2 Validar que tenga una query param
        - 3 filtrar la info de la bs o memoria con la query param
        - 4 Devolver 200 con la info filtrada
...
npm i @types/node -D
npm install multer convert-csv-to-json (multer es un midleware para manejar la subida de archivos. se puede usar para guardar en el server el archivo o guardarlo en memoria con buffer)
npm i @types/multer -D
npm install sonner (toast)
npm i @uidotdev/usehooks (debounce: para guardar lo que se busca cada x tiempo y no realizar busquedas por cada input de tecla)